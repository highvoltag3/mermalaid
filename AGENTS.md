...
- **The `mcp/` folder is a separate, self-contained Node package** (`@mermalaid/mcp`, the AI-agent bridge server, also installable via `npx -y @mermalaid/mcp`) with its own `package.json`, lockfile, `tsconfig.json` (NodeNext, emitting), and Vitest config — it is **not** part of the root install or build. Build/test it with `npm run mcp:build` (root) or `cd mcp && npm install && npm test`. The root ESLint/TS/Vitest configs are scoped to `src/**` and ignore it. The editor's `src/agentBridge/protocol.ts` is a verbatim copy of `mcp/src/protocol.ts`, guarded by `src/agentBridge/protocol.drift.test.ts` — edit both together. See [`docs/AGENT_INTEGRATION.md`](docs/AGENT_INTEGRATION.md).
